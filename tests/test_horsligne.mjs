...

{
  ...
  t('injecterPrecache : laisse le reste intact',
    injecte.includes('const CACHE_NAME = `aba-${CACHE_VERSION}`;'), true);
}
...

{
  ...
  const store = env.magasins.get('aba-vInstall1');
...

{
  ...
  env.seed('aba-vAncien', { './': null, './index.html': null });
  env.seed('aba-vNeuf', { './': null, './index.html': null }); // complet
  await declencher(env, 'activate');
  t('activate : purge les anciens caches quand le nouveau est complet',
    (await env.caches.keys()).sort(), ['aba-vNeuf']);
...

{
  ...
  env.seed('aba-vAncien', { './': null, './index.html': null, './assets/a.js': null });
  env.seed('aba-vIncomplet', { './': null }); // il manque index.html et assets/a.js
  await declencher(env, 'activate');
  t('activate : ne purge rien si le nouveau cache est incomplet',
    (await env.caches.keys()).sort(), ['aba-vAncien', 'aba-vIncomplet'].sort());
}
...

{
  ...
  env.seed('aba-vEtat', { './': null, './index.html': null }); // ./assets/a.js manque
  let recu;
